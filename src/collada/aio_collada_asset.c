...
aio_dae_assetInf(xmlTextReaderPtr __restrict reader,
  ...
  do {
    ...
    if (_xml_eqElm(_s_dae_contributor)) {
      ...
      contrib = aio_calloc(sizeof(*contrib), 1);
      ...
      do {
        ...
        if (_xml_eqElm(_s_dae_author))
          _xml_readText(contrib->author);
        else if (_xml_eqElm(_s_dae_author_email))
          _xml_readText(contrib->author_email);
        else if (_xml_eqElm(_s_dae_author_website))
          _xml_readText(contrib->author_website);
        else if (_xml_eqElm(_s_dae_authoring_tool))
          _xml_readText(contrib->authoring_tool);
        else if (_xml_eqElm(_s_dae_comments))
          _xml_readText(contrib->comments);
        else if (_xml_eqElm(_s_dae_copyright))
          _xml_readText(contrib->copyright);
        else if (_xml_eqElm(_s_dae_source_data))
          _xml_readText(contrib->source_data);
        else
          _xml_skipElement;
