...
dae_mesh(DAEState   * __restrict dst,
  ...
  while (xml) {
    ...
    } else if (xml_tag_eq(xml, _s_dae_triangles)) {
    } else if (xml_tag_eq(xml, _s_dae_trifans)) {
    } else if (xml_tag_eq(xml, _s_dae_tristrips)) {
  ...
  do {
    ...
    switch (found->val) {
      ...
      case k_s_dae_polylist: {
        ...
        ret = dae_poly(xst, memPtr, found->key, mode, &polygon);
        if (ret == AK_OK) {
          if (last_prim)
