...
fspool_c.write("""
#ifndef _ply_STRPOOL_
#  define _ply_STRPOOL_
#endif

#include "strpool.h"
#include <string.h>

const char _s_ply_pool_0[] =
""".encode())
...
for name, val in spool.items():
  ...
  headerContents.append("#define _s_ply_{0} _s_ply_{1}({2})\n"
...
for idx in range(spidx + 1):
  fspool_h.write("\n_AK_EXTERN const char _s_ply_pool_{0}[];"
                   .format(str(idx)).encode())

fspool_h.write("\n\n".encode())

for idx in range(spidx + 1):
  fspool_h.write("#define _s_ply_{0}(x) (_s_ply_pool_{0} + x)\n"
                   .format(str(idx)).encode())
