...
for name, val in spool.items():
  ...
  headerContents.append("#define _s_gltf_{0} _s_gltf_{1}({2})\n"
                        "#define _s_gltf_{0}_len {3}\n"
                          .format(name, str(spidx), str(pos), str(len(val))))
  if len(val) <= 4 and val.isascii():
    headerContents.append("#define _s_gltf_{0}_u32 0x{1:08x}u\n"
                            .format(name, u32_ascii_ci(val)))
    headerContents.append("#define _s_gltf_{0}_u32_exact 0x{1:08x}u\n"
                            .format(name, u32_ascii(val)))
  if len(val) <= 8 and val.isascii():
    headerContents.append("#define _s_gltf_{0}_u64 0x{1:016x}ull\n"
                            .format(name, u64_ascii_ci(val)))
    headerContents.append("#define _s_gltf_{0}_u64_exact 0x{1:016x}ull\n"
